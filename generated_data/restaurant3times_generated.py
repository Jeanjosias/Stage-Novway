...
def resto3times(Nombre_client, Accompagneur_max, Temps_ouverture, Table_disponible, cuisinier_disponible, performance) : 
    
    client = [i+1 for i in range(Nombre_client)]
    systeme = pd.DataFrame({"client" : client})
    
    pti_dej_client = Nombre_client//10
    din_client = Nombre_client//15
    dej_client = Nombre_client - (pti_dej_client+din_client)
    
    pti_dej_arrive = random.sample(range(0,Temps_ouverture//8), pti_dej_client)
    dej_arrive = random.sample(range(Temps_ouverture//6, Temps_ouverture*8//9), dej_client)
    din_arrive = random.sample(range(Temps_ouverture*10//11, Temps_ouverture), din_client)
    
    Instant_arrive = pti_dej_arrive + dej_arrive + din_arrive
    ...
    Accompagnant = systeme[["client", "Accompagnant"]] 
    Instant_arrive = systeme[["client", "arrival"]] 
